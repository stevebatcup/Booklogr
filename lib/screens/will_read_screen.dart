...
class WillReadScreen extends StatelessWidget {
  static String pageRoute = '/will-read';
  ...
  Widget build(BuildContext context) {
    ...
    Widget appBar = isMobile
        ? AppBarMobile(
            loggedIn: loggedIn,
            selectedTabName: 'Will read',
          )
        : AppBarDesktopTablet(activeSection: NavBarSection.willRead);

    return ResponsiveBuilder(
      builder: (context, sizingInformation) {
        if (loggedIn) {
          BooksCollection<WillReadBook> collection =
              BooksCollection<WillReadBook>()..initUserCollection(user);
          return Scaffold(
            backgroundColor: Theme.of(context).colorScheme.background,
            appBar: appBar,
            body: CentredView(
              child: Consumer<List<WillReadBook>>(
                builder: (context, bookList, child) {
                  ...
                  if (bookList.length == 0) {
                    return EmptyListAction(
                      ...
                      label: "Run a search to add books your WILL READ list",
                      onTap: () {
          ...
          return Scaffold(
            ...
            body: CentredView(
              child: EmptyListAction(
                ...
                label:
                    "Sign up to Booklogr to add books to your WILL READ list",
                onTap: () {
                  isMobile
